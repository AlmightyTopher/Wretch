...
const AdminGalleryEditor: React.FC = () => {
  ...
  const [imageTitle, setImageTitle] = useState<string>(""); // For optional title
  const [imageDescription, setImageDescription] = useState<string>(""); // For optional description
  ...
  const handleUpload = async () => {
    ...
    try {
      // Consider generating a unique ID for the filename to prevent overwrites
      ...
      const newImageData: NewGalleryImageData = {
        ...
        title: imageTitle || selectedFile.name, // Use provided title or default to filename
        description: imageDescription || "",
        // uploadedAt will be set by the service
      };
      ...
      setImageTitle("");
      setImageDescription("");
      // Clear file input visually (though this is notoriously tricky across browsers)
  ...
  const handleDelete = async (imageId: string, filename: string) => {
    ...
    if (window.confirm(`Are you sure you want to delete ${filename}? This action cannot be undone.`)) {
      ...
      try {
        ...
        await deleteImageFromFirebaseStorage(storagePath);

        console.log(`Image file ${filename} deleted from storage and Firestore.`);
  ...
  // ... existing code ... <loading and admin check>
      <div className="mb-4 p-4 border rounded shadow-sm bg-gray-50">
        ...
        <div className="space-y-3">
          <input
            id="gallery-file-input" // Added ID for clearing
            type="file"
            accept="image/*"
        ...
        {selectedFile && (
          <div className="mt-3">
            ...
            <img src={URL.createObjectURL(selectedFile)} alt="Preview" className="mt-2 rounded max-h-40 border" />
          </div>
        )}
        <button
          ...
          {galleryImages.map((image) => (
            <li key={image.id} className="flex items-center justify-between">
              <div className="flex items-center">
                <img src={image.downloadURL} alt={image.filename} className="w-10 h-10 rounded" />
                <p className="ml-2 text-sm text-gray-700">{image.filename}</p>
              </div>
              <button
