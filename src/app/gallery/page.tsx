...
export default function GalleryPage() {
  const [galleryImages, setGalleryImages] = useState<GalleryImage[]>([]); // Use GalleryImage type
  const [lightboxImage, setLightboxImage] = useState<GalleryImage | null>(null); // Store full image object for lightbox
    ...
    <section className="relative flex flex-col items-center min-h-[80vh] w-full overflow-hidden p-4 md:p-8">
      ...
      <div className="w-full max-w-6xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 md:gap-8">
        {galleryImages.map((image) => (
          <button
            key={image.id || image.downloadURL} // Use id if available, otherwise fallback to URL
          ...
          >
            <NextImage
              src={image.downloadURL} // Use downloadURL from Firestore
              alt={image.title || image.filename} // Use title or filename for alt text
              ...
              unoptimized={true} // If images are from Firebase Storage, they might not need Next.js optimization, or configure domains in next.config.js
              priority={galleryImages.indexOf(image) < 4} // Prioritize loading for the first few images
            />
